  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (response) {
        ...
        const userResponse = await fetchName(email);
        console.log("User data fetched:", userResponse);
        if (userResponse?.hasCheckedSymptoms) {
          navigation.navigate('HomeScreen', { email }); // Pass email to HomeScreen
        } else {
          navigation.navigate('SymptomCheck', { email }); // Pass email to SymptomCheck
        }
